...
// ✅ AUTOCOMPLETE (keep simple or reuse service later)
...
// ✅ ADVANCED (AI enhanced but no routing)
...
// 🚀 MAIN AI SEARCH (your smart feature)
export const advancedSmartSearch = async (req, res) => {
  try {
    const { query } = req.body;
    ...
    // 🔥 Routing
    if (intentData.intent === "horoscope") {
      return res.json({ type: "redirect", route: "/horoscope" });
    }

    if (intentData.intent === "market") {
      return res.json({ type: "redirect", route: "/market" });
    }

    if (intentData.intent === "weather") {
      return res.json({ type: "redirect", route: "/weather" });
    }

    if (intentData.intent === "news") {
      return res.json({ type: "redirect", route: "/news" });
    }
